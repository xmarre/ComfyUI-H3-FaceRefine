...
- Fixed CUDA InsightFace environments silently regressing to CPU after dependency reconciliation. FaceRefine now revalidates ONNX Runtime in `prestartup_script.py` after ComfyUI-Manager's dependency phase and before normal custom-node imports; if `CUDAExecutionProvider` disappeared, the existing `onnxruntime-gpu` version is force-reinstalled last and verified in a fresh interpreter.
- Changed the ORT repair policy to coexist with other custom nodes that explicitly require the CPU-named `onnxruntime` distribution (including WD14 Tagger). CPU distribution metadata may remain installed; the enforced invariant is that the GPU module payload wins and exposes `CUDAExecutionProvider`, avoiding an install/remove loop across launches.
- Fixed the CUDA backend guard being swallowed by `tracking_fixes`' optional InsightFace fallback. Identity-required tracking now validates the backend before entering that recoverable exception boundary, so a broken CUDA ORT backend cannot silently continue as motion-only tracking with `identity anchor frames=0`.
- CUDA ComfyUI hosts refuse accidental CPU-only InsightFace identity fallback instead of spending tens of seconds on a misconfigured backend or silently weakening subject identity. Deliberate CPU identity use remains available through `H3FACEREFINE_ALLOW_CPU_IDENTITY=1`; genuine CPU hosts are unchanged.
...
- Added VAE round-trip cancellation: clean and refined crop latents are decoded together and only `decode(refined_latent) - decode(clean_latent)` is transferred onto the tracked source crop.
